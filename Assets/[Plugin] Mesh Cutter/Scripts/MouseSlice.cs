...
public class MouseSlice : MonoBehaviour {
	...
	public EventSlice onSlicedMesh = new EventSlice();
	...
	void DrawPlane(Vector3 position, Vector3 rotation)
	{
	...
	#region API
	public void Slice(GameObject obj, Vector3 point, Vector3 normalVec)
	{
		...
		var transformedNormal = ((Vector3)(obj.transform.localToWorldMatrix.transpose * normalVec)).normalized;
		...
		if (SliceObject(ref slicePlane, obj, positive, negative, normalVec))
			SeparateMeshes(positive, negative, normalVec);
	}
    ...
    void SliceObjects(Vector3 point, Vector3 normal)
    {
        ...
        for (int i = 0; i < toSlice.Length; ++i)
        {
            ...
            slicedAny = SliceObject(ref slicePlane, obj, positive, negative, normal) || slicedAny;
        }
    ...
    bool SliceObject(ref Plane _slicePlane, GameObject obj, List<Transform> positiveObjects, List<Transform> negativeObjects, Vector3 normal)
    {
        var mesh = obj.GetComponent<MeshFilter>().mesh;

        if (!meshCutter.SliceMesh(mesh, ref _slicePlane))
        {
            // Put object in the respective list
            if (_slicePlane.GetDistanceToPoint(meshCutter.GetFirstVertex()) >= 0)
        ...
        GameObject newObject = Instantiate(obj, obj.transform.parent);
		...
		Vector3 vecTemp = obj.transform.position - newObject.transform.position;
		if (vecTemp.sqrMagnitude > (vecTemp + normal).sqrMagnitude) {
			OnSlicedMesh(obj, true);
			OnSlicedMesh(newObject, false);
		} else {
			OnSlicedMesh(obj, false);
			OnSlicedMesh(newObject, true);
		}
	...
	public void OnSlicedMesh(GameObject obj, bool positive)
	{
		onSlicedMesh.Invoke(obj, positive);
	}
